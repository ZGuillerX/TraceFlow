    ...
    <TraceFlowShell workspace>
      <div className="px-5 py-8 lg:px-10 lg:py-10">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          ...
          <Link
            href="/workspace"
            className="button-press inline-flex items-center justify-center gap-2 bg-[#1652F5] px-4 py-3 text-sm font-bold text-white hover:bg-[#0B3ECB]"
          >
            <ArrowRight size={15} /> Nuevo proyecto
          </Link>
        </div>
        <div className="mt-8 flex flex-col gap-3 border-y border-[#DEDDD3] py-4 sm:flex-row">
          <div className="flex flex-1 items-center gap-2 border border-[#DEDDD3] bg-white px-3 py-2 text-sm text-[#9AA1B2]">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 border border-[#DEDDD3] bg-white px-3 py-2 text-xs font-bold text-[#0C1330]"
          >
            <Filter size={15} /> Filtrar{" "}
            <span className="text-[#9AA1B2]">Todos</span>
          </button>
        </div>
        <div className="relative mt-10 flex min-h-[380px] flex-col items-center justify-center overflow-hidden border border-dashed border-[#DEDDD3] bg-white px-6 text-center">
          <div className="pointer-events-none absolute left-12 top-10 grid grid-cols-4 gap-2 opacity-30">
            {Array.from({ length: 12 }).map((_, i) => (
              <span
                key={i}
                className={`h-2.5 w-2.5 ${i % 3 === 0 ? "bg-[#1652F5]" : "bg-[#0C1330]"}`}
              />
            ))}
          </div>
          <HistoryEmptyIllustration className="pointer-events-none absolute bottom-10 right-10 h-24 w-44 opacity-20" />
          <div className="relative mb-6">
            <div className="relative flex h-20 w-20 items-center justify-center border border-[#DEDDD3] bg-[#FAF9F5] text-[#1652F5]">
              <FolderOpen size={30} strokeWidth={1.6} />
            </div>
          </div>
          <div className="eyebrow">Todavía no hay proyectos</div>
          <h2 className="mt-3 font-display text-2xl font-semibold tracking-[-.04em] text-[#0C1330]">
            Tu trabajo más limpio está por llegar.
          </h2>
          <p className="mt-3 max-w-[400px] text-sm leading-relaxed text-[#7A8194]">
          ...
          <Link
            href="/workspace"
            className="mt-7 inline-flex items-center gap-2 border border-[#DEDDD3] bg-white px-4 py-3 text-sm font-bold text-[#0C1330] hover:border-[#1652F5]"
          >
